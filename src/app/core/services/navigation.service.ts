...
export class NavigationService {
  ...
  private navigationItems: NavigationItem[] = [
    ...
    },
    {
      ...
      description: 'Empresas y aliados que apoyan OAB',
      route: '/patrocinadores',
      keywords: ['patrocinadores', 'patrocinios', 'aliados', 'sponsors', 'empresas', 'apoyo'],
      section: 'Navegación',
      icon: 'award'
    },
    
    // COMENTADO TEMPORALMENTE - Autenticación y cuenta
    // {
    ...
    // COMENTADO TEMPORALMENTE - Carrito de compras
    // {
    ...
    // COMENTADO TEMPORALMENTE - Filtros y ordenamiento de productos
    // {
  ...
  searchResults = computed(() => {
    ...
    if (!query) {
      // Si no hay búsqueda, mostrar accesos rápidos principales (solo páginas disponibles)
      return this.navigationItems.filter(item => 
        ['Inicio', 'Servicios', 'Nosotros', 'Patrocinadores'].includes(item.title)
      ).slice(0, 6);
    }
